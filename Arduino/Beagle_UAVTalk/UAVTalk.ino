...
#ifdef DEBUG
void uavtalk_show_msg(uavtalk_message_t *msg) {
	uint8_t *d;
	uint8_t i;
	uint8_t c;
	uint8_t crc;
	...
	Serial.print(millis()); Serial.print(" header ");
        c = (uint8_t) (msg->Sync);
	//osd.printf("%2x ", c);//
...
void uavtalk_send_msg(uavtalk_message_t *msg) {
	...
	if (op_uavtalk_mode & UAVTALK_MODE_PASSIVE)
        {
#ifdef DEBUG
	   Serial.print("No tengo UAVTalk activado!");
#endif
           return;
	}
...
int uavtalk_read(void) {
	...
	// grabbing data
	while (!show_prio_info && SERIAL_PORT.available() > 0) {
