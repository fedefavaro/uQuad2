...
int open_port_CC3D(char *device)
{
...
int configure_port_CC3D(int fd, speed_t baudrate) //TODO cambiar nombre
{
...
bool check_read_locks_CC3D(int fd) {
...
int uavtalk_init(void)
{
   ...
   int ret = configure_port_CC3D(fd, B115200);
   if (ret < 0)
      return -1;
...
int uavtalk_parser_read_char(int fd_CC3D, actitud_t *act)
{
   int retval = 0;
   ...
   if (retval <= 0) {
	...
	if (retval > 0) {
	   // consume msg
	   if (msg.ObjID == ATTITUDESTATE_OBJID)
	   {
...
int uavtalk_parser_start(struct timeval main_start)
{
   ...
   if (child_pid == 0)
   {
	...
	for(;;)
	{
	   ...
	   CC3D_readOK = check_read_locks_CC3D(fd_CC3D);
	   if (CC3D_readOK) {
		act_updated = uavtalk_parser_read_char(fd_CC3D, &act);
		CC3D_readOK = false;
	   }
